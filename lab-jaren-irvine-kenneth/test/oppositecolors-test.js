...
describe('testing oppositecolors transform', function() {
  let outputFileName = `${__dirname}/../outputs/oppositecolors-test.bmp`;
...
//==>>  Although the colorArray does change, this test doesn't think so. WHYYY?
      expect(transformer.oppositeColors.bind(transformer.oppositeColors, this.bitmap)).to.change(this.bitmap, 'colorArray');
  ...
  describe('testing with invalid input', () => {
    ...
    it('should throw an error', () => {
      expect(transformer.oppositeColors).to.throw('expected a bitmap buffer');
    });
  });
});
